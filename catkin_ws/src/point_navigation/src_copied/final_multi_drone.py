...
def setup():
    global pub,sub,destination_points, current_destination_pointer,drone_name
    global current_destination_point,current_position,current_yaw,yaw_precision,dist_precision
    
    drone_name = sys.argv[1]
    
    pub = rospy.Publisher("/"+drone_name+"/cmd_vel",Twist,queue_size=1)
    sub = rospy.Subscriber("/"+drone_name+"/ground_truth_to_tf/pose",PoseStamped,callback_odom)
    
    #GETTING LIST OF DESTINATION POINTS
    des_string = rospy.get_param("/"+drone_name+'/des_points')
...
def change_destination_point():
    ...
    print("DRONE: "+drone_name+" POINT: X:"+str(current_destination_point.x)+" Y: "+str(current_destination_point.y)+" EXPLORED")
    #CURRENT DESTIANTION PTR Indicator should be incremented
    if(current_destination_pointer == len(destination_points) -1):
        print("DRONE: "+drone_name+"ALL POINTS EXPLORED")
        change_state(3)
        return
...
def takeoff():
    ...
    twist_msg = Twist()    
    twist_msg.linear.z = float(sys.argv[2])
    #print("Drone: "+drone_name+"Takeoff")
    pub.publish(twist_msg)
        
def main():

    global current_state,current_destination_point,pub,drone_name,mutex
    ...
    while not rospy.is_shutdown():
        ...
        #print("Drone: "+drone_name+" state: "+str(current_state))
        if current_state == 0:
            fix_yaw()
