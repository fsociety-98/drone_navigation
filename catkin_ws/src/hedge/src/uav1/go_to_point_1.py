...
def set_point_one(point):
    global explore
    print("EXPLORE SIZE: "+str(len(explore))+" * "+str(len(explore[0])))
    if point.x < len(explore) and point.y < len(explore[0]):
...
def fix_yaw(des_pos):
    ...
    #print("Position_ : y = "+str(position_.y)+" x = "+str(position_.x))
    #print("Destination : y = "+str(des_pos.y)+" x = "+str(des_pos.x))
    #print("Yaw: "+str(yaw_))
    ...
    #print("Error : "+str(math.fabs(err_yaw))+" Yaw_prec: "+str(yaw_precision_))
    #print("\n\n")
...
def main():
    ...
    pub = rospy.Publisher('/uav1/cmd_vel', Twist, queue_size=1)

    arrpub = rospy.Publisher('/uav1/array',Int32MultiArray,queue_size = 100)

    arrsub2 = rospy.Subscriber('/uav0/array',Int32MultiArray,clbk_array)
    arrsub3 = rospy.Subscriber('/uav2/array',Int32MultiArray,clbk_array)

    sub_odom = rospy.Subscriber('/uav1/ground_truth_to_tf/pose', PoseStamped, clbk_odom)
