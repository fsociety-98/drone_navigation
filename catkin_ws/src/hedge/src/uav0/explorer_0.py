...
def main():
    ...
    rospy.init_node('explorer0')

    sub_laser = rospy.Subscriber('/uav0/scan', LaserScan, clbk_laser)
    sub_odom = rospy.Subscriber('/uav0/ground_truth_to_tf', Odometry, clbk_odom)
    #pub = rospy.Publisher('/cmd_vel', Twist, queue_size=1)

    rospy.wait_for_service('/go_to_point_switch0')
    rospy.wait_for_service('/wall_follower_switch0')
    rospy.wait_for_service('/gazebo/set_model_state')

    srv_client_go_to_point_ = rospy.ServiceProxy('/go_to_point_switch0', SetBool)
    srv_client_wall_follower_ = rospy.ServiceProxy('/wall_follower_switch0', SetBool)
    srv_client_set_model_state = rospy.ServiceProxy('/gazebo/set_model_state', SetModelState)

    # set robot position
    model_state = ModelState()
    model_state.model_name = 'uav0'
